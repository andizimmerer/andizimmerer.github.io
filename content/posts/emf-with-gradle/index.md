...
The code examples in this post are provided in Kotlin. Converting them to Java, however, should be straight-forward.
...
# Integrate Xtext
Xtext is used as a parser-generator for your Domain Specific Language (DSL).
...
```groovy
implementation project(":mydsl")
```

TODO

## 3. Invoke the Xtext Parser from your existing Code
...
For another good blog post about this, see [Dave's Blog][standalone-xtext].
...
# Integrate QVT-o
QVT-o is used for Model-To-Model Transformations.

Our goal is:

 - Specify a transformation from one metamodel ("input") to another metamodel ("output") with a plain `.qvto` file in our `resources` folder.
 - Apply the QVT-o transformation defined in this file to the `EObject` we got from the parser.
 - Get the resulting `EObject` after the transformation. The new `EObject` should adhere to the "output" metamodel.

## Required Dependencies

Because QVT is relatively tightly integrated into the Eclipse platform, we need a lot of dependencies that come directly from Eclipse.

To do this, you first need to add the following two repositories to your `build.gradle`.
...
The dependencies you need to add to your `build.gradle` to run QVT without eclipse are:
...
Uff yeah, that's quite a lot!

The resulting `build.gradle` of Plantestic can be found [here on GitHub][plantestic-core-gradlefile].

## Invoke QVT-o from your Code

```kotlin
fun doQvtoTransformation(inputModel: EObject, transformationUri: URI): EObject {
    // Sources:
    ...
    // print logs from the transformation to STDOUT
    context.log = WriterLog(OutputStreamWriter(System.out!!))
...
# Integrate Acceleo
Acceleo is used for Model-To-Text Transformations.
...
[plantestic-core-gradlefile]: https://github.com/FionaGuerin/plantestic/blob/master/core/build.gradle "The core build.gradle file of Plantestic"